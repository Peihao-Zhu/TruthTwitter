...
router.post("/", authParser, (req, res) => {
  ...
  UserAccessor.findUserById(data.userId)
    .then((user) => {
      if (user) {
    ...
    .catch((error) =>
      res.status(404).send(`Something wrong happened:${error}`)
    );
});
...
router.delete("/:tweetId", authParser, (req, res) => {
  ...
  TweetAccessor.findTweetById(req.params.tweetId).then((tweet) => {
    if (tweet) {
      if (tweet.userId !== req.body.userId)
        res
          .status(404)
          .send("Yout don't have the permission to delete other user's post");
      else {
...
router.put("/:tweetId", authParser, (req, res) => {
  // check if the you are the one who post the tweet
  if (!mongoose.Types.ObjectId.isValid(req.params.tweetId)) {
    res.status(404).send("Tweet Id is invalid");
  }
  TweetAccessor.findTweetById(req.params.tweetId).then((tweet) => {
    if (tweet) {
      if (tweet.userId !== req.body.userId)
        res
          .status(404)
          .send("Yout don't have the permission to edit other user's post");
      else {
